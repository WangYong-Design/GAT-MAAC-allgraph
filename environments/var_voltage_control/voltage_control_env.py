...
class VoltageControl(MultiAgentEnv):
    ...
    def get_state(self):
        """return the global state for the power system
           the default state: voltage, active power of generators, bus state, load active power, load reactive power,pv_flag
        """
        state = []
        ...
        return state.reshape(self.obs_dim,-1).transpose()
    ...
    def _cal_state_adj(self):
        self.state_bus_num = self.get_num_of_buses()
        self.edg_index = []
        self.edge_pair = set()
        edge_idex_src = []
        edge_idex_trg = []

        for start_index in range(self.state_bus_num):
            dist = top.calc_distance_to_bus(self.base_powergrid,start_index,weight = None)
            dist_index = dist.index[dist <= self.args.dis_threshold]
            for end_index in range(self.state_bus_num):
                if end_index in dist_index  and  (start_index,end_index) not in self.edge_pair:
                    edge_idex_src.append(start_index)
                    edge_idex_trg.append(end_index)
                    self.edge_pair.add((start_index, end_index))

        self.edg_index.append(
            torch.stack((torch.tensor(edge_idex_src), torch.tensor(edge_idex_trg)), dim=0).to(self.device))


    def get_constraint_mask(self):
        return self.mask

    def get_bus2region(self):
        region = list(self.region)
        return np.array([region.index(zone_name) for zone_name in self.powergrid.bus['zone']])

    def get_agent_index_in_obs(self):
        return self.agent_index_in_obs

    def get_agent_index_in_state(self):
        return self.agent_idx_in_state

    def get_region_adj(self):
        return self.adj

    def get_state_edge_index(self):
        return self.edg_index
