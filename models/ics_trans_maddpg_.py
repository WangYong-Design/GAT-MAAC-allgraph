...
class ICSTRANSMADDPG(Model):
    def __init__(self,args,target_net = None):
        ...
        self.bus2region = args.bus2region   # region2number for every bus,example "zone1" to 0
        self.region_num = np.max(self.bus2region) + 1
        self.state_adj = args.state_adj
        ...
        self.batchnorm = nn.BatchNorm1d(self.args.n_).to(self.device)
    ...
    def construct_value_net(self):
        if self.args.critic_type =="GAT":
            if self.args.critic_encoder:
                from GAT.GAT_Critic_ex import GAT_Critic
                input_shape = self.args.hid_size + self.act_dim
                self.critic_dicts = nn.ModuleList([GAT_Critic(input_shape,self.args)])
            else:
                from GAT.GAT_Critic_ex import GAT_Critic
                ...
                self.critic_dicts = nn.ModuleList([MLPCritic(input_shape,1,self.args)])
        else:
             NotImplementedError()
    ...
    def encode(self,raw_obs):
        ...
        agent_index = torch.from_numpy(self.agent_index_in_state)[None,:,:].repeat(batch_size,1,1).contiguous().to(self.device)
    ...
    def value(self,raw_obs,act):
        ...
        if self.args.shared_params:
            agent_value = self.critic_dicts[0]
            data_list = []

            for i in range(raw_obs.shpae[0]):
                data_list.append(Data(inputs[i], edge_index=self.edg_index[i]))
            batch = torch_geometric.data.Batch.from_data_list(data_list).to(self.device)
    ...
    def get_loss(self,batch):
        ...
        return policy_loss, critic_loss, action_out, lambda_loss
