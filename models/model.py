...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.bus_num = self.args.bus_num
    ...
    def train_process(self, stat, trainer):
        ...
        # reset env
        _, global_state = trainer.env.reset()
        ...
        for t in range(self.args.max_steps):
            ...
            global_state_ = th.from_numpy(global_state).to(self.device)\
                .float().contiguous().view(1,self.bus_num,self.obs_bus_dim)
            with th.no_grad():
                action, action_pol, log_prob_a, _, hid = self.get_actions(global_state_, status='train', exploration=True, actions_avail=th.tensor(
                    trainer.env.get_avail_actions()), target=False, last_hid=last_hid)
                value = self.value(global_state_, action_pol)
            ...
            next_global_state_ = th.from_numpy(next_global_state).to(self.device).float().\
                contiguous().view(1,self.bus_num,self.obs_bus_dim)
            with th.no_grad():
                _, next_action_pol, _, _, _ = self.get_actions(next_global_state_, status='train', exploration=True, actions_avail=th.tensor(
                    trainer.env.get_avail_actions()), target=False, last_hid=hid)
                next_value = self.value(next_global_state_, next_action_pol)
            ...
            trans = self.Transition(global_state,
        ...
        # stat_train['mean_train_reward'] * float(t+1)
    ...
    def evaluation(self, stat, trainer, test_season='June'):
        ...
        with th.no_grad():
            for _ in range(num_eval_episodes):
                ...
                _, global_state = trainer.env.manual_reset(
                    test_data[_], 23, 2)
                # init hidden states
                last_hid = self.policy_dicts[0].init_hidden()
                for t in range(self.args.max_eval_steps):
                    global_state_ = th.from_numpy(global_state).to(self.device) \
                        .float().contiguous().view(1, self.bus_num, self.obs_bus_dim)
                    action, _, _, _, hid = self.get_actions(global_state_, status='test', exploration=False, actions_avail=th.tensor(
                        trainer.env.get_avail_actions()), target=False, last_hid=last_hid)
                ...
                # stat_test_epi['mean_test_reward'] *= (t+1)
