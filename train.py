...
parser.add_argument("--save-path", type=str, nargs="?", default="./trail",
...
parser.add_argument("--wandb",  type = bool,default='true',help = "Please input weather use the wandb")
...
# alg_config_dict["obs_size"] = env.get_state_size()
...
alg_config_dict['bus2region'] = env.get_bus2region()
alg_config_dict['agent_index_in_state'] = env.get_agent_index_in_state()
# alg_config_dict['region_adj'] = env.get_region_adj()
alg_config_dict["state_adj"] = env.get_state_edge_index()
# if argv.date_emb:
...
data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), argv.save_path)
